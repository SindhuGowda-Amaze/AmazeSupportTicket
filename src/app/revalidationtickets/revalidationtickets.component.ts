...
export class RevalidationticketsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.roleid == 3) {
      this.AmazeSupportService.GetSupportTickets().subscribe((data) => {
        debugger;
        this.ticketList = data.filter((x) => x.status == 'Reopened');
        this.stafflistCopy = this.ticketList;
      });
    } else {
      this.AmazeSupportService.GetSupportTickets().subscribe((data) => {
  ...
  public getCompanylist() {
    this.AmazeSupportService.GetSupportTickets().subscribe((data) => {
      debugger;
      let temp: any = data.filter((x) => x.status == 'Reopened');
  ...
  public close() {
    ...
    this.AmazeSupportService.UpdateCloseStatusSupportTickets(entity).subscribe(
      (data) => {
        debugger;
    ...
    if (this.companyName == 0) {
      ...
      this.AmazeSupportService.GetSupportTickets().subscribe((data) => {
        ...
        this.ticketList = data.filter(
          (x) => x.status == 'Reopened' && x.companyname == this.companyName
        );
      });
    }
  }
  ...
  public GetFilteredapplicationName() {
    if (this.applicationName == 0) {
      ...
      this.AmazeSupportService.GetSupportTickets().subscribe((data) => {
        ...
        this.ticketList = data.filter(
          (x) =>
            x.status == 'Reopened' && x.applicationName == this.applicationName
        );
      });
    }
  }
  ...
  public GetFilteredissuefrom(event: any) {
    ...
    this.AmazeSupportService.GetSupportTickets().subscribe((data) => {
      debugger;
      this.ticketList = data.filter(
        (x) => x.issuefrom == this.issuefrom && x.status == 'Reopened'
      );
    });
  }
